...
class FullyConnected:
    ...
    def __init__(self, N_in, N_h1, N_h2, N_out, device='cpu'):
        ...
        w1 = (2/N_in**0.5)*torch.rand(N_h1,N_in)+(-1/N_in**0.5)
        w2 = (2/N_h1**0.5)*torch.rand(N_h2,N_h1)+(-1/N_h1**0.5)
        w3 = (2/N_h2**0.5)*torch.rand(N_out,N_h2)+(-1/N_h2**0.5)
        self.weights = {'w1': w1, 'w2': w2, 'w3': w3}

        b1 = torch.zeros(N_in,1)
        b2 = torch.zeros(N_h1,1)
        b3 = torch.zeros(N_h2,1)
    ...
    def train(self, inputs, labels, lr=0.001, debug=False):
        ...
        accuracy = accuracy(self,outputs,labels)
    ...
    def forward(self, inputs):
        ...
        Returns:
            outputs (torch.tensor): predictions from neural network. Size (batch_size, N_out)
        """
        self.cache['z1'] = torch.mm(self.weights['w1'],inputs) + self.biases['b1']
        a1 = activation.sigmoid(self.cache['z1'])
        self.cache['z2'] = torch.mm(self.weights['w2'],a1) + self.biases['b2']
        a2 = activation.sigmoid(self.cache['z2'])
        self.cache['z3'] = torch.mm(self.weights['w3'],a2) + self.biases['b3']
    ...
    def backward(self, inputs, labels, outputs):
        ...
        d2 = activation.delta_sigmoid(self.cache['z2'])
        d1 = activation.delta_sigmoid(self.cache['z1'])
    ...
    def calculate_grad(self, inputs, d1, d2, dout):
        ...
        Returns:
            ...
            db3 (torch.tensor): Gradient of loss w.r.t. b3. Size like b3
        """
        dw3 = dout*activation.sigmoid(self.cache['z2'])
        dw2 = dout*self.weights['w3']*d2*activation.sigmoid(self.cache['z1'])
        dw1 = dout*self.weights['w3']*d2*self.weights['w2']*d1*inputs

        db1 = dout*self.weights['w3']*d2*self.weights['w2']*d1
        db2 = dout*self.weights['w3']*d2
        db3 = dout
        return dw1, db1, dw2, db2, dw3, db3
